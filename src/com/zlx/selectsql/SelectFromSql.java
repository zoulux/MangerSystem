...
public class SelectFromSql {
	...
	public  boolean deleteStudent(String stu) {
		...
		try {
			...
			stmt = con.createStatement();
			String sql = String
					.format("delete from score1 where sno='%s'", stu);
	...
	public boolean addStudent(UserBean stu) {
		...
		try {
			...
			stmt = con.createStatement();
			String sql = String
					.format("insert into score1 values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')",
	...
	public ArrayList<UserBean> selectAll() {
		...
		try {
			...
			stmt = con.createStatement();
			String sql = "";
			sql = String.format("select * from score1");
			rs = stmt.executeQuery(sql);
			while (rs.next()) {
	...
	public  ArrayList<UserBean> selectStudent(String data) {
		...
		try {
			...
			if (MyTools.isDigit(data)) {
				sql = String.format(
						"select * from score1 where sno like '%%%s%%' ", data);
			} else {
				sql = String
						.format("select * from score1 where sname like '%%%s%%' ",
								data);
			}
	...
	public static boolean updateStudent(UserBean stu) {
		...
		try {
			...
			stmt = con.createStatement();
			String sql = String
					.format("update score1 set sports='%s',shufen='%s',jiexi='%s',gaodai='%s',safe='%s',english='%s',englishNet='%s',lishi='%s',wuli='%s' where sno='%s'",
	...
	public static void main(String[] args) {
		UserBean userBean = new UserBean("1888888888", "邹鲁贤", "80", "80", "80",
				"80", "80", "80", "80", "80", "80");
	}
